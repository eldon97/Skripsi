...
Ide utama dari teknik \textit{Random Rotation Perturbation} adalah jika data direpresentasikan sebagai matrix \(X_{d \times n}\) , \textit{rotation perturbation} dari datasest X didefinisikan sebagai \(G(X)=RX\). Dimana \(R_{d \times d}\) adalah \textit{random rotation othonormal matrix}.
