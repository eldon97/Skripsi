...
\begin{equation}
	U = \frac{1}{\sqrt{k}\sigma_{r}}RX
\end{equation}
Dimana \(R_{k \times m}\) adalah \textit{random projection matrix} yang dihasilkan mengikuti distribusi standar normal, dengan rata-rata bernilai 0 dan standar deviasi bernilai \(1/\sqrt{k}\). Ukuran matriks \(R_{k \times m}\) disesuaikan dengan matriks \(X_{m \times n}\) yang mana dataset asli dengan jumlah rekord \(n\) dan jumlah attribut \(m\), yang mana \(m\) akan menjadi dimensi matriks. Oleh karena reduksi dimensi lah yang diinginkan maka \(k\) harus lebih kecil dari pada \(m\), yang mana \(k\) adalah dimensi dari matriks baru yang dihasilkan dari \textit{Random Projection Perturbation} ini.
